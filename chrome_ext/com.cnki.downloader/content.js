...
// 确保脚本只运行一次
if (window.hasOwnProperty('cnkiExtractorInitialized')) {
  console.log('[知网下载链接提取器] 脚本已经初始化，跳过重复执行');
} else {
  window.cnkiExtractorInitialized = true;
  console.log('[知网下载链接提取器] 初始化脚本');
  initExtractor();
}
...
function initExtractor() {
  ...
  chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.action === "extract") {
      ...
      // 模拟点击下载链接
      console.log(`[知网下载链接提取器] 收到模拟点击请求: ${request.title}`);
      const result = simulateClickDownload(request.url, request.title);
      sendResponse({success: result});
      return true;
    }
  });
  
  // 模拟点击下载链接 - 使用控制台中有效的那种简单方法
  function simulateClickDownload(url, title) {
    console.log(`[知网下载链接提取器] 模拟点击下载: ${title} - ${url}`);
    ...
    try {
      ...
      console.log(`[知网下载链接提取器] 成功模拟点击下载: ${title}`);
      return true;
    } catch (error) {
      console.error(`[知网下载链接提取器] 模拟点击下载失败: ${error.message}`);
      return false;
    }
  }
